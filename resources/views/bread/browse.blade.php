                                    ...
                                    @foreach($dataType->browseRows as $row)
                                        <td>
                                            ...
                                            @elseif($row->type == 'text')
                                                @include('dola::multilingual.input-hidden-bread-browse')
                                    ...
                                    <td class="no-sort no-click" id="bread-actions">
                                        @if (Dola::can('delete_'.$dataType->name))
                                            <a href="javascript:;" title="Delete" class="btn btn-sm btn-danger pull-right delete" data-id="{{ $data->id }}" id="delete-{{ $data->id }}">
                                                <i class="dola-trash"></i> <span class="hidden-xs hidden-sm">Delete</span>
                                            </a>
                                        @endif
                                        @if (Dola::can('edit_'.$dataType->name))
                                            <a href="{{ route('dola.'.$dataType->slug.'.edit', $data->id) }}" title="Edit" class="btn btn-sm btn-primary pull-right edit">
                                                <i class="dola-edit"></i> <span class="hidden-xs hidden-sm">Edit</span>
                                            </a>
                                        @endif
                                        @if (Dola::can('read_'.$dataType->name))
                                            <a href="{{ route('dola.'.$dataType->slug.'.show', $data->id) }}" title="View" class="btn btn-sm btn-warning pull-right">
                                                <i class="dola-eye"></i> <span class="hidden-xs hidden-sm">View</span>
                                            </a>
                                        @endif
                                    </td>
                                </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="dola-trash"></i> Are you sure you want to delete
                        this {{ strtolower($dataType->display_name_singular) }}?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('dola.'.$dataType->slug.'.index') }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
...
<link rel="stylesheet" href="{{ dola_asset('lib/css/responsive.dataTables.min.css') }}">
@endif
@stop
...
@section('javascript')
    <!-- DataTables -->
    @if(!$dataType->server_side && config('dashboard.data_tables.responsive'))
        <script src="{{ dola_asset('lib/js/dataTables.responsive.min.js') }}"></script>
    @endif
