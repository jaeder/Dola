...
@section('css')
    <link rel="stylesheet" type="text/css" href="{{ dola_asset('css/nestable.css') }}">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="dola-list"></i>Menu Builder ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="dola-plus"></i> New Menu Item</div>
    </h1>
    @include('dola::multilingual.language-selector')
@stop

@section('content')
    @include('dola::menus.partial.notice')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="dola-trash"></i> Are you sure you want to delete this menu item?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('dola.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}"
                          id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 id="m_hd_add" class="modal-title hidden"><i class="dola-plus"></i> Create a New Menu Item</h4>
                    <h4 id="m_hd_edit" class="modal-title hidden"><i class="dola-edit"></i> Edit Menu Item</h4>
                </div>
                <form action="" id="m_form" method="POST"
                      data-action-add="{{ route('dola.menus.item.add', ['menu' => $menu->id]) }}"
                      data-action-update="{{ route('dola.menus.item.update', ['menu' => $menu->id]) }}">
                    ...
                    <div class="modal-body">
                        ...
                        @include('dola::multilingual.input-hidden', ['_field_name' => 'title', '_field_trans' => ''])
                        ...
                        <label for="icon_class">Font Icon class for the Menu Item (Use a <a
                                    href="{{ dola_asset('fonts/dola/icons-reference.html') }}"
                                    target="_blank">Dola Font Class</a>)</label>
...
@section('javascript')
    ...
    <script type="text/javascript" src="{{ dola_asset('js/jquery.nestable.js') }}"></script>
    @if($isModelTranslatable)
        <script type="text/javascript" src="{{ dola_asset('js/multilingual.js') }}"></script>
    @endif
    <script>
        $(document).ready(function () {
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('dola.menus.order',['menu' => $menu->id]) }}', {
