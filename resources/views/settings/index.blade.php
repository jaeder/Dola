...
@section('css')
    <style>
        .panel-actions .dola-trash {
            cursor: pointer;
        }

        .panel-actions .dola-trash:hover {
            color: #e94542;
        }
...
@section('head')
    <script type="text/javascript" src="{{ dola_asset('lib/js/jsonarea/jsonarea.min.js') }}"></script>
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="dola-settings"></i> Settings
    </h1>
@stop
...
@section('content')
    ...
    <div class="container-fluid">
        @include('dola::alerts')
        @if(config('dola.show_dev_tips'))
        <div class="alert alert-info">
            <strong>How To Use:</strong>
            <p>You can get the value of each setting anywhere on your site by calling <code>Dola::setting('key')</code></p>
        </div>
        @endif
    </div>

    <div class="page-content container-fluid">
        <form action="{{ route('dola.settings.update') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
            <div class="panel">
                @foreach($settings as $setting)
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            {{ $setting->display_name }}<code>Dola::setting('{{ $setting->key }}')</code>
                        </h3>
                        <div class="panel-actions">
                            <a href="{{ route('dola.settings.move_up', $setting->id) }}">
                                <i class="sort-icons dola-sort-asc"></i>
                            </a>
                            <a href="{{ route('dola.settings.move_down', $setting->id) }}">
                                <i class="sort-icons dola-sort-desc"></i>
                            </a>
                            <i class="dola-trash"
                               data-id="{{ $setting->id }}"
                    ...
                    <div class="panel-body">
                        ...
                        @elseif($setting->type == "image" || $setting->type == "file")
                            @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::disk(config('dola.storage.disk'))->exists($setting->value))
                                <div class="img_settings_container">
                                    <a href="{{ route('dola.settings.delete_value', $setting->id) }}" class="dola-x"></a>
                                    <img src="{{ Storage::disk(config('dola.storage.disk'))->url($setting->value) }}" style="width:200px; height:auto; padding:2px; border:1px solid #ddd; margin-bottom:10px;">
                                </div>
        ...
        <div class="panel" style="margin-top:10px;">
            <div class="panel-heading new-setting">
                <hr>
                <h3 class="panel-title"><i class="dola-plus"></i> New Setting</h3>
            </div>
            <div class="panel-body">
                <form action="{{ route('dola.settings.store') }}" method="POST">
                    {{ csrf_field() }}
                    <div class="col-md-4">
                    ...
                    <div class="col-md-12">
                        <a id="toggle_options"><i class="dola-double-down"></i> OPTIONS</a>
                    ...
                    <script>
                        $('document').ready(function () {
                            $('#toggle_options').click(function () {
                                ...
                                if ($('#toggle_options .dola-double-down').length) {
                                    $('#toggle_options .dola-double-down').removeClass('dola-double-down').addClass('dola-double-up');
                                } else {
                                    $('#toggle_options .dola-double-up').removeClass('dola-double-up').addClass('dola-double-down');
                                }
                            });
                        });
                    </script>
                    ...
                    <button type="submit" class="btn btn-primary pull-right new-setting-btn">
                        <i class="dola-plus"></i> Add New Setting
                    </button>
                    <div style="clear:both"></div>
                </form>
            </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <i class="dola-trash"></i> Are you sure you want to delete the <span id="delete_setting_title"></span> Setting?
                    </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('dola.settings.delete', ['id' => '__id']) }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
...
@section('javascript')
    ...
    <form id="my_form" action="{{ route('dola.upload') }}" target="form_target" method="POST" enctype="multipart/form-data" style="width:0;height:0;overflow:hidden">
        {{ csrf_field() }}
        <input name="image" id="upload_file" type="file" onchange="$('#my_form').submit();this.value='';">
        <input type="hidden" name="type_slug" id="type_slug" value="settings">
    </form>

    <script src="{{ dola_asset('lib/js/tinymce/tinymce.min.js') }}"></script>
    <script src="{{ dola_asset('js/dola_tinymce.js') }}"></script>
    <script src="{{ dola_asset('lib/js/ace/ace.js') }}"></script>
    <script src="{{ dola_asset('js/dola_ace_editor.js') }}"></script>
    <script>
