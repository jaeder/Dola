...
use DFZ\Dola\Models\Translation;
use DFZ\Dola\Translator;
...
trait Translatable
{
    ...
    public function translations()
    {
        return $this->hasMany(Translation::class, 'foreign_key', $this->getKeyName())
            ...
            ->whereIn('locale', config('dola.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('dola.multilingual.locales', [config('dola.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('dola.multilingual.default'), false];
        }
        ...
        $default = config('dola.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('dola.multilingual.default', 'en');
        $locales = config('dola.multilingual.locales', [$default]);
    ...
    public function prepareTranslations(&$request)
    {
        ...
        foreach ($transFields as $field) {
            ...
            $request->merge([$field => $trans[config('dola.multilingual.default', 'en')]]);
