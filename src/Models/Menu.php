...
class Menu extends Model
{
    ...
    {
        return $this->hasMany(Dola::modelClass('MenuItem'));
    }
    ...
    public static function display($menuName, $type = null, array $options = [])
    {
        ...
        if (in_array($type, ['admin', 'admin_menu'])) {
            ...
            $dataTypes = Dola::model('DataType')->all();
            $prefix = trim(route('dola.dashboard', [], false), '/');
            ...
            if (is_null($type)) {
                $type = 'dola::menu.default';
            } elseif ($type == 'bootstrap' && !view()->exists($type)) {
                $type = 'dola::menu.bootstrap';
            }
        }
