...
use DFZ\Dola\Traits\AlertsMessages;
use Validator;
...
abstract class Controller extends BaseController
{
        ...
        switch ($row->type) {
            ...
            case 'file':
                if ($file = $request->file($row->field)) {
                    ...
                    $request->file($row->field)->storeAs(
                        ...
                        config('dola.storage.disk', 'public')
                    );
            ...
            case 'multiple_images':
                if ($files = $request->file($row->field)) {
                    ...
                    foreach ($files as $key => $file) {
                        ...
                        Storage::disk(config('dola.storage.disk'))->put($filePath, (string) $image, 'public');
                        ...
                        if (isset($options->thumbnails)) {
                            foreach ($options->thumbnails as $thumbnails) {
                                ...
                                Storage::disk(config('dola.storage.disk'))->put($path.$filename.'-'.$thumbnails->name.'.'.$file->getClientOriginalExtension(),
                                    (string) $image, 'public'
                                );
                            }
                        }
            ...
            case 'image':
                if ($request->hasFile($row->field)) {
                    ...
                    Storage::disk(config('dola.storage.disk'))->put($fullPath, (string) $image, 'public');
    ...
    {
        if (Storage::disk(config('dola.storage.disk'))->exists($path)) {
            Storage::disk(config('dola.storage.disk'))->delete($path);
        }
    }
}
