...
use DFZ\Dola\Database\DatabaseUpdater;
use DFZ\Dola\Database\Schema\Column;
use DFZ\Dola\Database\Schema\Identifier;
...
class DolaDatabaseController extends Controller
{
    public function index()
    {
        Dola::canOrFail('browse_database');

        $dataTypes = Dola::model('DataType')->select('id', 'name')->get()->pluck('id', 'name')->toArray();
        ...
        return view('dola::tools.database.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create()
    {
        ...
        return view('dola::tools.database.edit-add', compact('db'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (isset($request->create_model) && $request->create_model == 'on') {
                $modelNamespace = config('dola.models.namespace', app()->getNamespace());
                $params = [
                ...
                Artisan::call('dola:make:model', $params);
            ...
            return redirect()
               ->route('dola.database.edit', $table->name)
    ...
    protected function prepareDbManager($action, $table = '')
    {
        ...
        if ($action == 'update') {
            ...
            $db->formAction = route('dola.database.update', $table);
        } else {
            ...
            $db->formAction = route('dola.database.store');
        }
    ...
    public function addBread(Request $request, $table)
    {
        ...
        return view('dola::tools.database.edit-add-bread', $data);
    }
    ...
    public function storeBread(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('dola.database.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('dola.database.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function addEditBread($table)
    {
        ...
        $dataType = Dola::model('DataType')->whereName($table)->first();
        ...
        return view('dola::tools.database.edit-add-bread', compact('dataType', 'fieldOptions', 'isModelTranslatable'));
    }
    ...
    public function updateBread(Request $request, $id)
    {
        ...
        /* @var \DFZ\Dola\Models\DataType $dataType */
        try {
            $dataType = Dola::model('DataType')->find($id);
    ...
    public function deleteBread($id)
    {
        ...
        $data = Dola::model('DataType')->destroy($id)
        ...
        if (!is_null($dataType)) {
            Dola::model('Permission')->removeFrom($dataType->name);
        }
