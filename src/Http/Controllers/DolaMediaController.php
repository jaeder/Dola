...
class DolaMediaController extends Controller
{
    /** @var string */
    private $filesystem;
    ...
    {
        $this->filesystem = config('dola.storage.disk');
    }

    public function index()
    {
        Dola::canOrFail('browse_media');

        return view('dola::media.index');
    }
    ...
    private function getFiles($dir)
    {
        ...
        foreach ($storageFiles as $file) {
            if (empty(pathinfo($file, PATHINFO_FILENAME)) && config('dola.hidden_files')) {
                $files[] = [
